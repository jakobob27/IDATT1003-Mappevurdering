...
/**
 * The TrainDeparture class is responsible for storing information stored within a specific train-departure.
 * It also has the responsibility of making sure that the information given when creating a new train-departure is both valid and correctly formatted.
 * If given an invalid parameter when constructing an object of the class, it will throw a IllegalArgumentException with an explanatory message.
 */
public class TrainDeparture {
    /**
     * A final localTime object that describes the time the train departs in the format HH:mm
     * A LocalTime object will make it easier to both format and do operations regarding time.
     ...
     * It is a String because the task specified that the field is a text.
     * The field is final because a train-departure won't change its line mid-departure.
     */
    private final String line;
    /**
     * A final string with a unique number within a 24-hour window in the format "602", "45", "1951" etc.
     * It is a String because the task specified that the field is a text.
     ...
     * It is a String because the destination is a noun.
     ...
     * The other fields in this object are in their correct format as given that they are their designated type.
     * Throws IllegalArgumentExceptions with fitting messages if one of the parameters is not in the right format.
     * @param departureTime a localTime object that cannot contain time-units lower than minutes
     * @param line a String where the first character is not a number, but all the other characters is a number.
    ...
    private void validator(LocalTime departureTime, String line, String trainNumber){
        if(departureTime.getSecond()!=0 || departureTime.getNano()!=0) {
            throw new IllegalArgumentException("departureTime cannot contain time-units lower than minutes!");
        }
        ...
        for (int i=0; i<lastPart.length(); i++) {
            if (!Character.isDigit(lastPart.charAt(i))) {
                throw new IllegalArgumentException("Every character after the first for the line must be a number!");
            }
        }
     ...
     */
    public TrainDeparture(LocalTime departureTime, String line, String trainNumber, String destination) {
    ...
    /**
     * Another constructor for when you want to define the track when creating a TrainDeparture object.
     * Throws ill
     * @param departureTime a localTime object that cannot contain time-units lower than minutes
     * @param line a String where the first character is not a number, but all the other characters is a number.
     * @param trainNumber a String where all the characters are numbers
     ...
     */
    public TrainDeparture(LocalTime departureTime, String line, String trainNumber, String destination, int track) {
